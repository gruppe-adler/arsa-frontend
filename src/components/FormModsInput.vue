...
watch(model, value => { localMods.value = Array.from(value); }, { immediate: true });
...
function removeMod() {
    ...
    const indices = Array.from(select.selectedOptions).map(o => o.index).reverse();
...
function importModset() {
    ...
    modset.forEach(mod => { if (!mod.required) mod.required = true; localMods.value.push(mod); });
    importText.value = '';
...
<style scoped>
.mods-container { align-items: start; }

.mods-wrap { display: flex; flex-direction: column; gap: 10px; width: 100%; }
...
.optional { color: var(--ink-4); font-size: 11px; }

/* Action row */
.mods-actions { display: flex; gap: 8px; flex-wrap: wrap; }
