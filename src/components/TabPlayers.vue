...
async function load() {
    loading.value = true;
    const players = await serversStore.getKnownPlayers(props.serverId);
    if (players) { knownPlayers.value = players; found.value = true; }
    loading.value = false;
}
...
const sorted = computed(() =>
    [...knownPlayers.value].sort((a, b) => a.name < b.name ? -1 : 1)
);
</script>
