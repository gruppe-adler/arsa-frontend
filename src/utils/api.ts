...
export async function getServers(): Promise<Server[]> {
    const jsonResponse = await fetch(`http://${api}:3000/api/get-servers`);
...
export async function startServer(uuid: string): Promise<boolean> {
    const jsonResponse = await fetch(`http://${api}:3000/api/server/${uuid}/start`);
...
export async function stopServer(uuid: string): Promise<boolean> {
    const jsonResponse = await fetch(`http://${api}:3000/api/server/${uuid}/stop`);
    const result = (await jsonResponse.json()) as Result;
    const logsStore = useLogsStore();
...
export async function isRunning(uuid: string): Promise<boolean> {
    const jsonResponse = await fetch(`http://${api}:3000/api/server/${uuid}/isRunning`);
    const result = (await jsonResponse.json()) as Result;
    //const logsStore = useLogsStore();
...
export async function getLogs(uuid: string): Promise<string[]> {
    const jsonResponse = await fetch(`http://${api}:3000/api/server/${uuid}/logs`);
...
export async function getStats(uuid: string): Promise<DockerStats> {
    const jsonResponse = await fetch(`http://${api}:3000/api/server/${uuid}/stats`);
...
export async function addServer(server: Server): Promise<string> {
    const jsonResponse = await fetch(`http://${api}:3000/api/add-server`, {
        method: 'POST',
...
export async function getPublicIp(): Promise<string> {
    const jsonResponse = await fetch(`http://${api}:3000/api/get-public-ip`);
