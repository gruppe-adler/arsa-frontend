...
function getLogs() {
    serversStore.getLogs(route.params.id as string).then(logs => {
        if (logs) { serverLogs.value = logs; found.value = true; }
        loading.value = false;
    });
}
...
<template>
  ...
  <main class="page-main" v-else>
    ...
    <div class="logs-table" v-if="sortedServerLogs.length > 0 || serverLogs.containsCrashReportsLog">
      <div class="log-row crash-row" v-if="serverLogs.containsCrashReportsLog">
      ...
      <div class="log-row" v-for="log in sortedServerLogs" :key="log.dir">
        ...
        <div class="log-links">
          <RouterLink class="log-link" :to="`/view-server-log/${route.params.id}/${log.dir}/console.log`" v-if="log.containsConsoleLog">console.log</RouterLink>
          <RouterLink class="log-link" :to="`/view-server-log/${route.params.id}/${log.dir}/error.log`"   v-if="log.containsErrorLog">error.log</RouterLink>
          <RouterLink class="log-link" :to="`/view-server-log/${route.params.id}/${log.dir}/script.log`"  v-if="log.containsScriptLog">script.log</RouterLink>
          <RouterLink class="log-link" :to="`/view-server-log/${route.params.id}/${log.dir}/crash.log`"   v-if="log.containsCrashLog">crash.log</RouterLink>
        </div>
